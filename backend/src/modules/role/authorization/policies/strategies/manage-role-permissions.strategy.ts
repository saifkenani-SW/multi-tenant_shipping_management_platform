...
import { ManageRolePermissionsPayload } from '../payloads';
import { RoleAuthorizationStrategy } from './interfaces/role-authorization-strategy.interface';

@Injectable()
export class ManageRolePermissionsStrategy implements RoleAuthorizationStrategy<ManageRolePermissionsPayload> {
  readonly action = RoleAction.ManagePermissions;
  ...
  async authorize(
    ...
    payload?: ManageRolePermissionsPayload,
  ): Promise<void> {
    if (!payload?.roleId) {
