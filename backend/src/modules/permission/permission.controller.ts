...
import type { IPermissionQueryService } from './interfaces/permission.query.service.interface';

/**
 * الكتالوج للقراءة فقط: لا توجد نقاط إنشاء أو تعديل أو حذف لأن كل
 * صلاحية يقابلها فحص في الكود، فمصدرها PERMISSION_CATALOG.
 */
@ApiTags('Permissions')
@ApiBearerAuth()
...
export class PermissionController {
  constructor(
    @Inject('IPermissionQueryService')
    private readonly permissionQueryService: IPermissionQueryService,
  ) {}
  ...
  async getPermissions(
    ...
    return this.permissionQueryService.findPermissions(query);
  }
  ...
  async getPermissionDetails(
    ...
    return this.permissionQueryService.getPermissionDetails(params.id);
  }
}
