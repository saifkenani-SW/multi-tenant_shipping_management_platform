async function run() {
  ...
  let accessToken = loginData.data ? loginData.data.accessToken : loginData.accessToken;
  let tenantId = null;
  if (loginData.status === 'REQUIRE_PROFILE_SELECTION') {
    console.log('Selecting profile...', loginData.profiles[0].id);
  ...
  console.log('Got access token! Tenant:', tenantId);
  
  console.log('Fetching a READY_FOR_COLLECTION parcel from DB via pg...');
  const { Client } = require('pg');
  const client = new Client({ connectionString: 'postgresql://postgres:postgres@localhost:51214/multi-tenant_shipping_management_platform' });
  await client.connect(); await client.query("DELETE FROM proof_of_delivery;");
  ...
  form.append('receivedByName', 'Sami Node 24');
  form.append('deliveryNotes', 'Left at door');
  
  // Create a Blob from the Buffer for the file upload
  const fileBlob = new Blob([Buffer.from('fake-signature-image')], { type: 'image/png' });
  form.append('signature', fileBlob, 'sig.png');
  ...
  const podRes = await fetch(`http://localhost:3000/parcels/${trackingNumber}/proof-of-delivery`, {
    method: 'POST',
    headers: headers,
    body: form
  });
